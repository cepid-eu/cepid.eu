...
const App = {
  ...
  initFilters() {
    ...
    // Job family checkboxes (sorted alphabetically)
    ...
    if (jobFamilyContainer) {
      ...
      jobFamilyContainer.innerHTML = families.map(family => `
        <label>
          <input type="checkbox" value="${family}" class="filter-job-family-cb">
          <span style="color: ${JobClassifier.getColor(family)};">●</span> ${family}
        </label>
      `).join('');

      jobFamilyContainer.querySelectorAll('.filter-job-family-cb').forEach(cb => {
        cb.addEventListener('change', () => this.applyFilters());
      });
    }

    // Contract type checkboxes (sorted: CDI first, then CDD by duration, then others)
    const contractContainer = document.getElementById('filter-contract');
    if (contractContainer) {
      const extractDuration = (contract) => {
        // Extract months: "CDD - 12 Mois" → 12
        const monthMatch = contract.match(/(\d+)\s*Mois/i);
        if (monthMatch) return parseInt(monthMatch[1]) * 30;
        // Extract days: "CDD - 15 Jours" → 15
        const dayMatch = contract.match(/(\d+)\s*Jours?/i);
        if (dayMatch) return parseInt(dayMatch[1]);
        return 0;
      };

      const contracts = Object.keys(data.byContractType).sort((a, b) => {
        // CDI first
        if (a === 'CDI') return -1;
        if (b === 'CDI') return 1;
        // CDD sorted by duration (descending - longest first)
        const isCDDA = a.startsWith('CDD');
        const isCDDB = b.startsWith('CDD');
        if (isCDDA && isCDDB) {
      ...
      // Show all contract types (exhaustive breakdown)
      contractContainer.innerHTML = contracts.map(type => `
        <label>
          <input type="checkbox" value="${type}" class="filter-contract-cb">
          ${type}
        </label>
      `).join('');

      contractContainer.querySelectorAll('.filter-contract-cb').forEach(cb => {
        cb.addEventListener('change', () => this.applyFilters());
      });
    }

    // Setup scroll indicators for filter groups
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', () => {
        document.querySelectorAll('.filter-job-family-cb, .filter-contract-cb')
        ...
        if (deptDropdown) deptDropdown.hidden = true;
        this.clearSelectedDepartments();
        DataProcessor.resetFilters();
      });
    }

    // Dispatch event signaling filters are ready (for BottomSheet to clone)
    window.dispatchEvent(new CustomEvent('filtersReady'));
    console.log('[App] Filters initialized and ready');
  },
