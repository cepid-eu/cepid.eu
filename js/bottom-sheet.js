...
const BottomSheet = (function() {
  ...
  function init() {
    ...
    if (!checkboxes || checkboxes.length === 0) {
      ...
      // Listen for filtersReady event and re-initialize
    ...
    if (jobFamilyContainer) {
      jobFamilyContainer.addEventListener('change', (e) => {
        if (e.target.type === 'checkbox') {
          ...
          if (e.target.checked) {
            if (!pendingFilters.jobFamilies.includes(value)) {
              pendingFilters.jobFamilies.push(value);
            }
          } else {
    ...
    if (contractContainer) {
      contractContainer.addEventListener('change', (e) => {
        if (e.target.type === 'checkbox') {
          const value = e.target.value;
          if (e.target.checked) {
            if (!pendingFilters.contractTypes.includes(value)) {
              pendingFilters.contractTypes.push(value);
            }
          } else {
